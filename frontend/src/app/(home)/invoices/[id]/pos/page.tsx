...
export default function PosTerminalPage() {
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        if (inv?.customerId) {
          try {
            const custRes = await apiGet<{ success: boolean; data: { name: string; phone?: string } }>(
              `/api/v1/customers/${inv.customerId}?businessId=${encodeURIComponent(businessId!)}`,
              { token }
            );
            if (!cancelled) {
              const cust = custRes?.data ?? (custRes as unknown as { data?: { name?: string; phone?: string } })?.data;
              setCustomerName(cust?.name ?? "");
              setCustomerPhone(cust?.phone?.trim() || null);
            }
          } catch { /* optional */ }
        }
  ...
  const handleWhatsAppShare = async () => {
    if (!invoicesApi || !invoiceId || !businessId) return;
    setWhatsappError(null);
    setWhatsappLoading(true);
    try {
      const url = await invoicesApi.getWhatsAppLink(invoiceId, businessId);
        ...
        <div className="w-full max-w-xs rounded-2xl border border-stroke bg-white p-4 shadow-sm dark:border-dark-3 dark:bg-gray-dark">
          ...
          <div className="flex flex-col gap-2">
            ...
            <button
              ...
              disabled={whatsappLoading || !customerPhone}
              title={!customerPhone ? t("pos.customerNoPhone") : undefined}
              className="flex items-center justify-center gap-2 rounded-lg border border-green-600 px-4 py-2.5 text-sm font-medium text-green-600 hover:bg-green-50 dark:border-green-500 dark:text-green-400 dark:hover:bg-green-900/20 disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {whatsappLoading ? (
              ...
              {t("pos.shareViaWhatsApp")}
            </button>

            {!customerPhone && invoice?.customerId && (
              <p className="rounded-lg bg-amber-50 p-2 text-xs text-amber-800 dark:bg-amber-900/20 dark:text-amber-300">
                {t("pos.customerNoPhone")}{" "}
                <Link href="/customers" className="font-medium underline hover:no-underline">
                  {t("pos.customerNoPhoneLink")}
                </Link>
              </p>
            )}
            ...
            {payUrl && (
              <button
                ...
                className="flex items-center justify-center gap-2 rounded-lg border border-stroke bg-white px-4 py-2.5 text-sm font-medium text-dark hover:bg-gray-2 dark:border-dark-3 dark:bg-dark-2 dark:text-white"
              >
                {linkCopied ? (
                ...
                {linkCopied ? t("pos.linkCopied") : t("pos.copyLink")}
              </button>
            )}

            {whatsappError && customerPhone && (
              <p className="rounded-lg bg-amber-50 p-2 text-xs text-amber-800 dark:bg-amber-900/20 dark:text-amber-300">
                {whatsappError}
              </p>
            )}
