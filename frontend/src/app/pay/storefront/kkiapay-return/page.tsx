...
import { confirmStorefrontKkiaPay } from "@/services/storefront.service";

function StorefrontKkiaPayReturnContent() {
  ...
  useEffect(() => {
    ...
    confirmStorefrontKkiaPay(token, transactionId, intentId, redirectStatus ?? undefined)
      .then((result) => {
        if (result.success) {
          setStatus("success");
        } else {
...
export default function StorefrontKkiaPayReturnPage() {
  return (
    <Suspense
      fallback={
    ...
    >
      <StorefrontKkiaPayReturnContent />
    </Suspense>
  );
}
